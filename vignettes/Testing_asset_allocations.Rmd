...
This vignette illustrates some ways to use the `AssetAllocation` package to backtest simple asset allocations. Example 1 shows how to backtest one of the pre-loaded asset allocations in the `asset_allocations` object.
...
The package comes with returns data to test all strategies in `asset_allocations` using ETFs. The returns are in the `ETFs` data set. Type `?ETFs` for details.
...
In this example, we test and compare four tactical asset allocation strategies: the Ivy Portfolio, the Robust Asset Allocation strategy, and the Dual Momentum strategy.
